...
"""
    ...
    def glm_chat_function(self, text_input, api_key, model_name, temperature, top_p, max_tokens, system_prompt_override, text_system_prompt_preset):
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                #"seed": ("INT", {"default": 0,"min": 0,"max": 0xffffffffffffffff,"step": 1,"display": "number","tooltip": "设置为0时，每次运行生成随机种子；设置为其他值时，使用固定种子。注意：此种子仅影响ComfyUI节点内部的随机数生成，不直接影响智谱AI模型的输出结果。"}),
            },
            "optional": {
                "image_url": ("STRING", {"default": "","placeholder": "请输入图片URL (与Base64/IMAGE三选一)"}),
                #"image_base64": ("STRING", {"multiline": True,"default": "","placeholder": "请输入Base64编码的图片数据 (与URL/IMAGE三选一)"}),
                "image_input": ("IMAGE", {"optional": True, "tooltip": "直接输入ComfyUI IMAGE对象 (与URL/Base64三选一)"}), # 新增IMAGE输入
            }
        }

    def generate_prompt(self, api_key, prompt_override, model_name, image_url="",image_base64 ="", image_prompt_preset="", image_input=None):
        final_api_key = api_key.strip() or get_zhipuai_api_key()
        if not final_api_key:
            _log_error("API Key 未提供。")
            return ("API Key 未提供。",)
